...
class AutoTrader(BaseAutoTrader):
    ...
    def __init__(self, loop: asyncio.AbstractEventLoop, team_name: str, secret: str):
        ...
        self.etf_bids, self.etf_asks, self.futures_asks, self.futures_bids = (
        ...
        with open("output/inputs.csv", "w", newline='') as f:  # DELETEME
            writer = csv.writer(f)
            writer.writerow(['position', 'hedged', 'avg_price', 'market_state', 'bid_liquidity',
                            'bid_spread', 'bid_lot', 'ask_liquidity', 'ask_spread', 'ask_lot'])
    ...
    def print_status(self):  # DELETEME
        ...
        with open('output/logs.txt', 'a') as f:
            f.write(
                f"Asks: {self.etf_asks}, Ask base: {self.ask_base}, Ask shifted: {self.ask_shifted}\n")
            f.write(
                f"Bids: {self.etf_bids}, Bid base: {self.bid_base}, Bid shifted: {self.bid_shifted}\n")
    ...
    def on_order_book_update_message(self, instrument: int, sequence_number: int, ask_prices: List[int],
        ...
        if instrument == Instrument.FUTURE:
            if self.do_nothing:
                self.check_market_state(ask_prices, ask_volumes, bid_prices, bid_volumes)
                return
            ...
            if self.check_market_state(ask_prices, ask_volumes, bid_prices, bid_volumes):
                return
    ...
    def check_market_state(self, ask_prices, ask_volumes, bid_prices, bid_volumes):
        """Checks the market state to decide on taking action"""
        if self.do_nothing:
            avg_price = (ask_prices[0] + bid_prices[0]) / 2
            self.calc_lot_sizes(avg_price, ask_prices, ask_volumes, bid_prices, bid_volumes)
            self.market_state = self.define_market_state()
            if abs(self.market_state) > 0.5:
                self.do_nothing = False
            return False
        elif abs(self.market_state) < 0.5:
            self.log("")
            self.log(f"Market state is too low: {self.market_state}, do nothing")
            self.log("")
            # if self.position > 0:
    ...
    def reset_orders(self, order_set, side, lot, price):
        ...
        total = abs(self.etf_position + (lot if side == Side.BUY else -lot))
        ...
        if abs(self.etf_position) > HEDGED_THRESHOLD:
            ...
            if lot_size > 0:
                order = Order(next(self.order_ids),
                              MAX_ASK_NEAREST_TICK, lot_size, 0)
                self.send_hedge_order(order.id, Side.BID,
                                      order.price, order.lot)
                self.futures_bids[order.id] = order
            else:
                order = Order(next(self.order_ids),
                              MIN_BID_NEAREST_TICK, -lot_size, 0)
                self.send_hedge_order(order.id, Side.ASK,
                                      order.price, order.lot)
                self.futures_asks[order.id] = order
        else:
            if self.futures_position > 0:
                order = Order(next(self.order_ids), MIN_BID_NEAREST_TICK, int(
                    self.futures_position), 0)
                self.send_hedge_order(order.id, Side.ASK,
                                      order.price, order.lot)
                self.futures_asks[order.id] = order
            elif self.futures_position < 0:
                order = Order(next(self.order_ids), MAX_ASK_NEAREST_TICK,
                              int(-self.futures_position), 0)
                self.send_hedge_order(order.id, Side.BID,
                                      order.price, order.lot)
                self.futures_bids[order.id] = order
